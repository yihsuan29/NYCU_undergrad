...
from RL_LAB4_109705001_陳以瑄.src.models.CarRacing_model import ActorNetSimple, CriticNetSimple
from RL_LAB4_109705001_陳以瑄.src.environment_wrapper.CarRacingEnv import CarRacingEnvironment
import random
...
class CarRacingTD3Agent(TD3BaseAgent):
	def __init__(self, config):
		...
		self.min_action = torch.tensor([-1, 0, 0]).to(self.device)
		self.max_action = torch.tensor([1, 1, 1]).to(self.device)
	...
	def decide_agent_actions(self, state, sigma=0.0, brake_rate=0.015):
		...
		with torch.no_grad():
			...
			action = self.actor_net(state).cpu().numpy().flatten() + sigma * self.noise.generate()
		# print(action)
		# print(action.shape)
		return action
