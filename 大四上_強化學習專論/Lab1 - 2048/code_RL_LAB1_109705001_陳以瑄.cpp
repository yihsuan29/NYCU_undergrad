...
public:
  ...
  std::vector<float>scores_rec;
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				// TODO
				//estimate return  = R_t+1 + sum P(s,a,s')*V(S_t+1)
				board af_s = move->after_state();
				af_s.popup();
				move->set_value(move->reward() + estimate(af_s));
	...
	void update_episode(std::vector<state>& path, float alpha = 0.1) const {
		...
		for(;path.size();path.pop_back()){
			state& t = path.back();
			float TD_target = t.reward()+V_t1;
			float TD_error = TD_target-estimate(t.before_state());
			V_t1 = update(t.before_state(),alpha* TD_error);
		}
	}
  ...
  void save_rec(const std::string& path) {
		std::string fileName = path;    
		std::ofstream outputFile(fileName);
		if (outputFile.is_open()) {
			for (const double& score : scores_rec) {
				outputFile << score << ",";
			}
			// Close the file
			outputFile.close();
...
int main(int argc, const char* argv[]) {
	...
	//tdl.save("weight0922_1_40.bin");
  	//tdl.save_rec("score0922_1_40.txt");
