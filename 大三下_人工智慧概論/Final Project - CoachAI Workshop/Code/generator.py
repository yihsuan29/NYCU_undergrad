...
if __name__ == "__main__":
    ...
    for rally_id in tqdm(testing_rallies):
        ...
        selected_matches = val_matches.loc[(val_matches['rally_id'] == rally_id)][:].reset_index(drop=True)
        ...
        for key in feature_name:
            given_seq[key] = torch.tensor(selected_matches[key].values).to(device)
        given_seq['target_player'] = target_players.to(device)
        given_seq['rally_length'] = generated_length
        """=========================================================="""
        # feed into the model
        generated_shot, generated_area = shotgen_generator(given_seq=given_seq, encoder=encoder, decoder=decoder, config=config, samples=SAMPLES, device=device,feature_name = feature_name)
